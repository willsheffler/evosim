...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--width", type=float, default=20.0, help="World width (continuous units)")
    parser.add_argument("--height", type=float, default=20.0, help="World height (continuous units)")
    parser.add_argument("--seed", type=int, default=7, help="Random seed for reproducible start")
    parser.add_argument("--ticks", type=int, default=10, help="How many steps to run")
    ...
    parser.add_argument("--fps", type=int, default=6, help="Frames per second in pygame mode")
...
def main() -> None:
    ...
    config = SimulationConfig(width=args.width, height=args.height, seed=args.seed, speed=args.speed)
    ...
    print(
        f"Start: tick={sim.tick}, {format_creature_position(sim.creature)}, "
        f"world={sim.world.width:.1f}x{sim.world.height:.1f}, seed={args.seed}, speed={args.speed}"
    )
    for _ in range(args.ticks):
        sim.step()
        if args.show_coords:
            print(f"Tick {sim.tick:>2}: {format_creature_position(sim.creature)}")
