...
This is a homeschool learning project.
Participants:
- Student Judah (13, basic Python, excellent math skills)
- Father/teacher Will (scientist/software engineer)
Primary goals:
- Teach core programming and simulation concepts.
- Have fun building evolving creature simulations together.
- Make scientific observations about simulation results.
...
- First milestone: a 2D board where creatures move and interact.
...
- Use pygame-ce for gui elements and graphics
- Use pytest for testing.
- Ensure simulations can be run and tested independently of GUI/graphics.
...
- There should be at least one quiz with 5 questions each session.
- At least one function or class should be written by the student each session.
- For each added feature, complete one evidence cycle: run and pass relevant tests, make a git commit for that feature, and capture at least one screenshot showing the feature working.
...
A session is considered done when all of the following are true:
...
- Why it changed
- How it was verified
- What student should learn from this step (brief)
